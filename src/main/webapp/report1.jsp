...
<%
    List<Candidate> candidates = (List<Candidate>) request.getAttribute("dsCAN");
    candidates = candidates !=null?candidates:new CandidateRespository().getAll();
%>
<body>
<form action="hello-servlet">
    ...
    <select class="form-control" id="role" name="role">
        ...
        <option value=<%=Role.ADMINISTRATION%>><%=Role.ADMINISTRATION%></option>
        <option value=<%=Role.STAFF%>><%=Role.STAFF%></option>
        <option value=<%=Role.MANAGER%>><%=Role.MANAGER%></option>
        <option value=<%=Role.EXECUTIVE%>><%=Role.EXECUTIVE%></option>
    </select>
    <button type="submit" name="viewRole">view</button>
</form>
<table class="table">
    <thead>
    <tr>
